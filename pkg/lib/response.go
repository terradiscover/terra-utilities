...
var patterns []sqlPatterns = []sqlPatterns{
...
// ErrorNotAllowed send http 405 forbidden
func ErrorNotAllowed(c *fiber.Ctx, message ...string) error {
	if len(message) == 0 {
		message = append(message, "Not Allowed")
	}

	return Send(c, 405, message[0])
}

// ErrorGone send http 410 gone
func ErrorGone(c *fiber.Ctx, message ...string) error {
	if len(message) == 0 {
		message = append(message, "Gone")
	}
	return Send(c, 410, message[0])
}
...
// ErrorUnauthorized send http 401 unauthorized
...
// ErrorTimeout send http 408 Request timeout
func ErrorTimeout(c *fiber.Ctx, message ...string) error {
	if len(message) == 0 {
		message = append(message, "Request Timeout")
	}

	return Send(c, 408, message[0])
}
...
// Custom Response Loyalty Account
func ErrorConflictLoyaltyAccount(c *fiber.Ctx, message ...interface{}) error {
	if len(message) == 0 {
		message = append(message, "Conflict")
	}
